...
export class DebugPage extends BasePage {
  ...
  async getLogEntry(index: number): Promise<{
    ...
    return {
      level: await entry.locator('[data-testid="log-level"]').textContent() || '',
      message: await entry.locator('[data-testid="log-message"]').textContent() || '',
      timestamp: await entry.locator('[data-testid="log-timestamp"]').textContent() || '',
    };
  }
}
