...
export class MetricsPage extends BasePage {
  ...
  async getMetric(index: number): Promise<{
    ...
    return {
      label:
        (await card.locator('[data-testid="metric-label"]').textContent()) ||
        '',
      value:
        (await card.locator('[data-testid="metric-value"]').textContent()) ||
        '',
      timestamp:
        (await card
          .locator('[data-testid="metric-timestamp"]')
          .textContent()) || '',
    }
  }
  ...
  async getCacheStatistics(): Promise<{
    ...
    return {
      ...
      size:
        (await this.cacheStatistics
          .locator('[data-testid="cache-size"]')
          .textContent()) || '',
      evictions:
        (await this.cacheStatistics
          .locator('[data-testid="cache-evictions"]')
          .textContent()) || '',
    }
  }
