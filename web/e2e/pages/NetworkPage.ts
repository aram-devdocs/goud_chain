...
export class NetworkPage extends BasePage {
  ...
  async getPeerInfo(index: number): Promise<{
    ...
    return {
      address:
        (await peer.locator('[data-testid="peer-address"]').textContent()) ||
        '',
      status:
        (await peer.locator('[data-testid="peer-status"]').textContent()) || '',
      lastSeen:
        (await peer.locator('[data-testid="peer-last-seen"]').textContent()) ||
        '',
    }
  }
