...
export class CollectionsPage extends BasePage {
  ...
  get searchInput(): Locator {
    return this.page.locator('input[name="search"], input[placeholder*="Search"]');
  }
  ...
  async getCollectionName(index: number): Promise<string> {
    return await this.collectionItems.nth(index).locator('[data-testid="collection-name"]').textContent() || '';
  }
  
  async getCollectionItemCount(index: number): Promise<number> {
    const text = await this.collectionItems.nth(index).locator('[data-testid="item-count"]').textContent() || '0';
    return parseInt(text.match(/\d+/)?.[0] || '0');
  }
  
  async openDecryptModal(collectionIndex: number): Promise<void> {
    await this.collectionItems.nth(collectionIndex).locator('button:has-text("Decrypt")').click();
