...
test.describe('Data Submission', () => {
  ...
  test('should submit data with collection name', async ({ authenticatedPage }) => {
    const testData = `Test data ${Date.now()}`;
  ...
  test('should show error for empty data submission', async ({ authenticatedPage }) => {
    // Attempt to submit empty data
    await submitPage.submitButton.click();
    
    // Should show validation error
    const errorMessage = await submitPage.page.locator('[role="alert"]').textContent();
    expect(errorMessage).toBeTruthy();
  });
  
  test('should submit multiple data items sequentially', async ({ authenticatedPage }) => {
    const dataItems = [
