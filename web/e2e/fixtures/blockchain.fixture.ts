  ...
  blockchain: async ({ page }, use) => {
    ...
    const blockchain = {
      ...
      async waitForBlock(blockHeight: number, timeout = 30000): Promise<void> {
        ...
        throw new Error(
          `Blockchain did not reach height ${blockHeight} within ${timeout}ms`
        )
      },
      ...
      async getBlock(height: number): Promise<any> {
        ...
        if (height >= data.blocks.length) {
          throw new Error(
            `Block ${height} does not exist (chain length: ${data.blocks.length})`
          )
        }
      ...
      async waitForBlockWithData(
        ...
        throw new Error(
          `Block with data ID ${dataId} not found within ${timeout}ms`
        )
      },
    }
